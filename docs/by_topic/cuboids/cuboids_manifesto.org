...
Additionally, another potential benefit lk;;lkllkklk;lkjk serverless costs are variable while self-magaged
services with constantly running servers have higher fixed costs. Minimizing
self-managed services can lead to lower costs, depending on use case.
...
The following tasks are broken out into isolatable chucks of work. The
plan is to spin up a Boss deploy and then treat each of the followings
tasks as a GitHub Issues Project. The Projects are to be worked on
individually so that overall progress can be broken out to milestones,
testable along the way via integration tests.
...
schema is currently in the repository named [[https://github.com/jhuapl-boss/boss][jhuap-boss/boss]]. That
machinery is to be replaced with Amazon API Gateway. Any Python logic
in the repo will be migrated to Lambdas.

API Gateway's was originally created to provide HTTP interfaces to AWS
...
(Actually it looks like API Gateway had just been released at the time
...
Cuboids will maintain the architecture, that is, auth as a separate
...
will be API Gateway reading from a Swagger file that mimics the Boss
...
Cognito User Pool. This means Cuboids can know who the existing
bossdb.io user are and can create Cognito user roles for them while in
Cuboids, running analysis on data in the MICrONS Boss repository.
...
existing machinery is more auto-scaling EC2 instances:
