...
deal with the sporadic, massive volume and velocity of data being
...
license. The codebase was [[https://github.com/jhuapl-boss/boss/graphs/contributors][initiate in 2015]] and has continued
...
Boss Lambdas and Step Functions.
...
Lambdas; some states might be long running Activities, say, an EC2
...
lots of Step Functions but they deal primarily with Lambdas, not long
...
contact with the enemy. For example, what to do about [[https://github.com/jhuapl-boss/boss-manage/blob/master/docs/InstallGuide.md#configure-scalyr-account][the Scalyr
logging]]. The next phase, actually deploying a stock bossDB, is to be
documented in boss_deploy_log.org.

It is assumed that these changes to bossDB are significantly
disruptive enough that PRs back to bossDB would not be worth the
hassle to the repository's maintainers. But maybe some chunks of code
will go upstream, say, the heaviside replacement code. Perhaps
upstream would be interested in the Step Function that works with
chunkflow for neuron reconstruction; being novel that part would not
involve major changes to existing code but that could be seen as
feature creep.

This development roadmap may well lead to a fork of the Boss
codebase. Forking is undesirable but may be inevitable. Although the
main thrust of this proposal is simply an architectural refresh
...
departure from the current roadmap. Nonetheless, a eye will be kept
out for areas where interaction can be maintained. For example, a PR
which simply replaces heaviside with AWS CDK code may be welcome
upstream.
...
child of serverless. The time is right to move the Boss back to the
