...
*The existing library of 18 Step Functions in the Boss can be seen as a library of code containing serverless components 
which perform fundimental CRUD and visualization operations on cuboids in a Boss repository, utilizing a scalable cache.* 
Analysis programs which run close to the
data can be built atop those base operators. The platform is already
there but it has not yet been exploited to the level where non-trivial
data pipelines have been built atop it. Cubiods is laying the
foundation for more interesting pipelines moving forward.

This section has two parts: first, an explanation of the design
pattern, serverless-first, which is being followed for building new
components in Cuboids. Then the design pattern will be put to use to
add neuron reconstruction machinery to Cuboids.
...
The core design innovation is that the architecture is being
re-oriented to be serverless-first. This section explains what is
meant by that.
   
The main AWS service than enables serverless-first designs is Step Functions,
which has already been adopted into the Boss machinery. There are
already at least [[https://github.com/jhuapl-boss/boss-manage/tree/master/cloud_formation/stepfunctions][18 Step Functions]]. For example, the Downsample service
...
based components. A Step Function program can be exposed in an API
Gateway hosted REST API. 
...
REST APIs. Inside AWS, The HTTP messages containing Boss REST
...
innately stateless AWS Lambda service. Any state which needs to
bridges both serverless and non-serverless processes is maintained in
a Step Function. The states -- as Lambdas and Activities -- are the
program modules which get assembled into a Step Functions based
program. The Boss Downsampler is the poster child program written atop
the Boss libraries.
...
design pattern is that there is a safety valve. The goal is to build
out as much as can be in a serverless fashion based on Lambdas, but
failing that various non-serverless components can be added into a
Step Function as needed. Same Step Function, but with some state
transitions driven by EC2 instances acting as Task Activities, rather
than all states being implemented as Lambdas.
...
considering when scale gets in the range of 10M monthly hits.

That Step Functions can orchestrate both serverless and server-based
compute is not obvious from the Boss codebase. The Boss has lots of
StepFunctions but they deal primarily with Lambdas, not long running
processes as task Activities.

Reviewing some implementation points touched upon earlier, Cuboids
will continue to build more Step Functions based programs that run
within the platform.  Any new Step Functions based code will not use
Heaviside, rather they will be written using the Python AWS
CDK. (Note: that does not mean that de-Heavisiding the codebase is a
prerequisite to such novel processes. CDK can live peacibly alongside
Heaviside.) 

This will make for a serverless first architecture. 

** Reconstruction: serverless CNN inference

The first major functional extension to be added to Cubiods is
...
Chunkflow. The novel part is that it will need to be deployed in a
...
research applying CNNs to connectomics. It seems the lab has been at it
[[https://www.simonsfoundation.org/2017/09/11/using-artificial-intelligence-to-map-the-brains-wiring/][since around 2006]]. 

The lab puts out a lot of software for connectomic data
processing. Most of it is open source, and a good deal of that is
licenced in a commerically friendly way (Apache, MIT, BSD, etc.).
...
to fold some Seung Lab code into Cubiods so that it has all the
...
The following montage illustrates what the Seung Lab code does. First
individual 2D "grayscale" images are segmented as indicated by the
superimposed colors. Then 3D models are constructed by stacking the
images.

#+attr_html: :width 100%
[[./images/seung_triptych.png]]
[[https://www.simonsfoundation.org/2017/09/11/using-artificial-intelligence-to-map-the-brains-wiring/][Via Seung Lab]]
...
code packages including machine learning libraries such as
...
bossDB-based system, which was not easy to do, if even possible,
in 2015.
...
segmentation to individual neurons, synapses, cetc. There are masters
...
make something for TensorFlow, following [[https://github.com/seung-lab/chunkflow/blob/master/chunkflow/chunk/image/convnet/patch/pytorch.py][class
...
in the pre-print seems to correspond to Chunkflow Issue #55 [[https://github.com/seung-lab/chunkflow/issues/55][use
pytorch/dask/tensorflow for parallel execution]] of the Chunkflow repo
on GitHub.
...
Step Functions for reconstruction jobs to perform. 

The ability to switch from Lambdas to EC2 instances may come
in handy in some use cases if demand is high and steady.

That is still not serverless-first but could be a component in a
StepFunction. So experiment first with Lambda chunkflow workers, then
if that goes well figure out how to take the master code and shoehorn
that into Lamba somehow. That would be the full serverless
implementation achieved in stages.

There needs to be a way for econstruction jobs to initiable. That
...
This is how an external API call can initiate a CNN job inside a Step Function.

This chuckflow plan illustrates how Step Functions can involve both
Lamba and EC2 instances. That's why it's worth implement services as
Step Functions. 
