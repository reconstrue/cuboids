...
the bossDB volumetric database. The envisioned platform has been given
the working title of Cuboids. Cuboids is a variant of bossDB: the AWS
locked-in, serverless-first, next generation spin off which performs
both data storage and analysis in one system.
...
pipelines. Examples include reconstruction inference using algorithms
...
thrown off by electron microscopes ([[https://www.nature.com/articles/s41467-020-18659-3][3 gigapixel per second]]), an AWS
