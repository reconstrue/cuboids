...
structured architecture enables individual component to be modified over
time.
...
the Apache 2.0 license in the hopes of building an community around
the codebase.
...
MICrONS. [ *TODO:* doubt it. Wasn't that 2D segmentaion? Or is that still what is done in 
...
The final state of development would be for to have a Lambda which is
...
learning of cell morphologies (analogously to human bodyes: locally,
...
maybe throw in some HDBSCAN clustering.  Use the top three priciple
components to define a voxel's RGB color. This illustrateds how novel
UI features can be added by multi-step analysis pipelines, whih would
